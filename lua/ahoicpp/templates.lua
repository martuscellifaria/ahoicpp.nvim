...
def main():
    ...
    cmake_config.write_text(f"""# Auto-generated by fetch_libpqxx.py
...
""")
    ...
    libpqxx_block = """target_include_directories({{PROJECT_NAME}} PRIVATE ${LIBPQXX_INCLUDE_DIR} ${LIBPQ_INCLUDE_DIR})
target_link_directories({{PROJECT_NAME}} PRIVATE ${LIBPQXX_LIB_DIR} ${LIBPQ_LIB_DIR})
target_link_libraries({{PROJECT_NAME}} pqxx pq)"""
    ...
    if app_cmake.exists():
        ...
        if "LIBPQXX_INCLUDE_DIR" not in content:
            ...
            for i in range(len(lines) - 1, -1, -1):
                if "target_link_libraries({{PROJECT_NAME}}" in lines[i]:
                    insert_idx = i + 1
                    break
            if insert_idx is not None:
                lines.insert(insert_idx, libpqxx_block)
                app_cmake.write_text("\n".join(lines) + "\n")
            else:
                with app_cmake.open("a") as f:
                    f.write(f"\n{libpqxx_block}\n")
...
def main():
    ...
    if not (opencv_output / "lib" / "libopencv_core.so").exists() and \
       ...
       not (opencv_output / "lib" / "opencv_core.lib").exists():
        ...
        cmake_args = [
            ...
            # Build only essential modules to save time
            ...
            # Disable other dependencies to keep it self-contained
            "-DWITH_GTK=OFF",
            "-DWITH_QT=OFF",
            "-DWITH_FFMPEG=OFF",
            "-DWITH_GSTREAMER=OFF",
        ...
        # Add contrib modules if available
    ...
    cmake_config.write_text(f"""# Auto-generated by fetch_opencv.py
set(OpenCV_DIR "{opencv_output}/lib/cmake/opencv4")
# Find OpenCV using the generated config file
...
""")
    ...
    app_cmake = root / "App" / "CMakeLists.txt"
    opencv_block = """# OpenCV integration
find_package(OpenCV REQUIRED PATHS "${OpenCV_DIR}" NO_DEFAULT_PATH)
...
target_link_libraries({{PROJECT_NAME}} ${OpenCV_LIBS})"""
    ...
    if app_cmake.exists():
        content = app_cmake.read_text()
        if "OpenCV_DIR" not in content:
            lines = content.splitlines()
            insert_idx = None
            for i in range(len(lines) - 1, -1, -1):
                if "target_link_libraries({{PROJECT_NAME}}" in lines[i]:
                    insert_idx = i - 1  # Insert before the last link libraries
                    break
            if insert_idx is not None:
                lines.insert(insert_idx, opencv_block)
                app_cmake.write_text("\n".join(lines) + "\n")
            else:
                with app_cmake.open("a") as f:
                    f.write(f"\n{opencv_block}\n")

    print("Done. Everything isolated in externals/")
